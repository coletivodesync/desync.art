...
void main() {
    ...
    color = mix(color, deepPurple, smoothstep(0.3, 0.7, combinedNoise + 0.5));
    color = mix(color, magenta, smoothstep(0.6, 1.0, combinedNoise + gradient * 0.5));
    color = mix(color, darkBlue, smoothstep(0.0, 0.4, gradient - combinedNoise * 0.3));

    // Add subtle brown tint in corners
    color = mix(color, brown, smoothstep(0.7, 1.0, gradient) * 0.2);
